...
| HU21-C | Gobernanza | Credenciales propias de la organización: guardar, recuperar, reasignar al reemplazo, sugerir rotación | `test_dashboard_credentials`, `test_me_credentials`, `test_secret_access`, `test_audit_chain` (cadena jsonb) + E2E real (`scripts/e2e_hu21_check.py`, 52/52 pasos sustantivos, `docs/evidencia/hu21-e2e.txt`) | ✅ | Cubierta en backend. UI de la extensión de esta etapa pendiente |
| HU18 | Gobernanza | Acceso administrativo con verificación TOTP (CU08): consultar y rotar la credencial de un integrante (AC1-AC5) | `test_totp_service` (vectores RFC 6238 Apéndice B, ventana, ambas capas del anti-replay, bloqueo), `test_mfa_enrollment`, `test_secret_access` (tripwire invertido, herméticos ampliados), `test_dashboard_credentials` (cadena real + piedra angular), `test_dashboard_vault`, `test_me_credentials`, `test_audit_chain`, `test_exceptions`, `test_vault_crypto` + E2E real (`scripts/e2e_hu18_check.py`, 35/35 pasos sustantivos, 6 omitidos, `docs/evidencia/hu18-e2e.txt`) | ✅ | Cubierta en backend. AC5 **medido**, no citado: el access token deja de servir en la petición siguiente, sin sesiones ni refresh tokens nuevos. UI de la extensión hecha en su repo (contrato real 13/13, `sparkgate-extension/docs/evidencia/hu18-contrato-real.txt`). Pendiente: verla en un Chrome real y tres manuales (KEK caída, `TOTP_MASTER_KEY` caída, app de autenticación real) |
...
| HU agregada después en Jira | 4 (HU16 SCRUM-25, HU17 SP-2, HU18 SCRUM-28, HU21 SP-2) |
| HU frontend (repo extensión, no aplica aquí) | 4 (HU08, HU13, HU14, HU15) |
| HU backend aplicables | 15 (11 del backlog original + HU16 + HU17 + HU18 + HU21) |
| HU backend cubiertas (✅) | 15 |
| HU backend parciales (⚠️) | 0 |
| **% cobertura backend** | **15/15 = 100%** |
